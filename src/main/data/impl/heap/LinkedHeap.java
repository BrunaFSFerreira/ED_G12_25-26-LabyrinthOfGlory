...
import main.data.adt.HeapADT;
import main.data.execption.EmptyCollectionExecption;
import main.data.impl.tree.LinkedBinaryTree;
...
public class LinkedHeap<T> extends LinkedBinaryTree<T> implements HeapADT<T> {
    ...
    /**
     * Returns the node that will be the last node after a remove
     * @return the node that will be the last node after a remove
     */
    ...
    /**
     * Reorders the heap after removing a node.
     */
    private void heapifyRemove() {
        ...
        if((left == null) && (right == null)) {
            ...
            next = left;
        } else if (((Comparable<T>) left.element).compareTo(right.element) < 0) {
            next = left;
        } else {
            next = right;
        }
